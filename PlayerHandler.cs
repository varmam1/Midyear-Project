...
public class PlayerHandler : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (Input.GetKeyDown(KeyCode.LeftControl) && (transform.localPosition.y < .11f) ) {
			print ("jump");
		...
		if (Input.GetKey (KeyCode.W)) {
			...
			velocity = transform.TransformDirection(new Vector3 (getSpeed (), 0, 0)) * getSpeed() * (-1.0f) * speedScale;
